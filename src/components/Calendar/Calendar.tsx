...
const HeaderFilter = ({
  ...
  <div className="bg-sombar text-white p-4 relative">
    <div className="flex justify-between items-center mb-2">
      ...
      {showBoardSelect && (
        <div className="absolute right-0 top-14 mt-2 bg-white text-black rounded-md shadow-lg p-4">
...
const TaskDateTable = ({ dates, cards }: TaskDateTableProps) => (
      ...
      return (
        <div key={date.toDateString()} className="flex flex-col bg-somon border border-som">
          ...
          <div className="flex-grow mt-2">
            {dailyCards.map((card) => (
              <div key={card.id} className="p-1 mt-1 rounded-md" style={{ backgroundColor: card.color }}>
...
export default function Calendar() {
  ...
  return (
    <div className="h-full bg-somon ml-64">
      ...
      />
      <TaskDateTable dates={dates} cards={cards} />
    </div>
  );
}
