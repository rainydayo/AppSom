...
export async function DELETE(req: Request) {
    try {
        ...
        if (idx_clb == -1) { // find card in list-board
            return NextResponse.json({ message: 'Cannot find card in list-board to update' }, {status: 404});
        }

        const idx_m = boardJson.data[idx_b].lists[idx_lb].cards[idx_clb].member.findIndex(m => m === card.uid);
        if (idx_m == -1) {
            return NextResponse.json({ message: 'Cannot find member' }, {status: 404});
        }
        boardJson.data[idx_b].lists[idx_lb].cards[idx_clb].member.splice(idx_m, 1);
